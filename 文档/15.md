...
## 项目初步重构
#### 原有问题
初步浏览我们去年写的项目发现，有些文件十分庞大，承担了过多的职责，导致代码高度耦合、难以维护。例如`ouside.cpp`文件，这个文件同时管理着UI显示(各种菜单、按钮、对话框)、玩家输入(键盘事件)、NPC逻辑、场景实体(动物、作物)、游戏逻辑(钓鱼)等等。我们首先需要对这些文件里的代码进行组件拆分和封装，便于我们后续进行重构。
#### 重构的必要性
1. **高昂的维护成本与“脆弱性”**
- **牵一发而动全身**：假设需要修改NPC的对话触发距离。这个逻辑埋在 `init()` 函数里一个巨大的lambda表达式中。在修改时，很可能无意中破坏了旁边毫不相干的签到按钮逻辑，或者影响了视口跟随的计算。
- **理解困难**：面对一个上千行的 `outside.cpp`，不了解这个项目的人很难快速理解系统的全貌。这极大地增加了上手时间和犯错的概率。因此我们需要进行初步的重构，便于之前不了解这个项目的队友能够更快上手。
2. **严重阻碍团队协作**
- **合并冲突**：在团队开发中，我们每个人有不同的分工，如果所有职责都在`outside.cpp`文件种实现，所有团队成员都工作在 `outside.cpp` 这个巨大的文件上，那么版本控制的合并冲突将频繁发生，且解决冲突会非常痛苦。
- **职责不清**：无法清晰地分配任务。“你负责游戏逻辑，我负责UI”这样的分工在当前的架构下无法实现，因为逻辑和UI深度耦合。
因此我们首先需要对这些文件里的代码进行组件拆分和封装，不仅提高代码质量，也便于我们后续合作重构代码。
#### 修改方案
我们对原有场景类中高度耦合、职责混杂的逻辑进行了系统性的拆分与封装，重新构建为以下几个相互独立、职责单一的功能模块：
1. **输入管理模块 (`InputManager`)**
- 将键盘事件处理从 `outside` 类中分离
- 专门处理 E 键触发 UI 切换的逻辑
- 提供统一的启用/禁用接口
```cpp
class InputManager {
...
2. **UI 管理模块 (`UIManager`)**
- 提取所有 UI 相关功能：个人界面、钓鱼界面、NPC 对话框
- 实现 `Observer` 接口接收时间更新事件
- 统一管理 UI 状态和生命周期
```cpp
class UIManager {
public:
...
3. **NPC 管理模块 (`NPCManager`)**
- 封装 NPC 的创建和交互逻辑
- 使用 `NPCSpec` 结构体配置 NPC 参数
- 处理距离检测和对话触发
```cpp
class NPCManager {
public:
...
```
5. **实体管理模块（`EntityManager`）**：
- 负责动物、作物、NPC、玩家等实体的创建
- 负责 `update` 调度（负责 `spawn`、统一 `update` 移除）
```cpp
#pragma once
#include "cocos2d.h"
...
#### 改进与收益
使用外观模式对场景管理进行重构后，系统在结构清晰度、可维护性和可测试性方面获得了显著提升：
* 场景切换逻辑原先分散在六个不同的文件中，导致修改和追踪困难；重构后，所有切换逻辑被统一收拢至 `GameSceneFacade` 类中，实现了集中管理。
* 重构前，各个场景类直接依赖 `Director` 以及彼此之间的具体实现，形成复杂的网状依赖；现在，场景类仅需依赖 `GameSceneFacade` 这一高层接口，大幅降低了模块间的耦合度。
* 原本每个场景类既要处理自身业务逻辑，又要负责加载、卸载、过渡等基础设施操作，职责不清；通过将这些通用流程交由 `GameSceneFacade` 统一处理，场景类得以专注于核心业务逻辑，符合单一职责原则。
* 由于逻辑高度分散且耦合紧密，旧架构下难以编写有效的单元测试；如今场景切换路径明确且封装良好，测试用例可以针对 `GameSceneFacade` 精准覆盖各种切换场景，显著提升了可测性与可维护性。
* 多个场景中重复出现的 `Director` 调用代码（如 `pushScene`、`popScene`、`replaceScene`）已被抽象并封装进 `Facade` 内部，不仅消除了冗余，还确保了调用行为的一致性和安全性。
...
1. **定义统一接口**：
为了让所有游戏场景都通过**统一的接口**进行创建，我们定义 `ISceneCreator` 作为场景构建的抽象基类。
```cpp
class ISceneCreator {
public:
...
```
- `createScene()`：负责创建具体场景实例，支持 `params` 参数。
- `getSceneType()`：返回场景类型字符串，如 `"Farm"` / `"Shop"`。
- `preloadResources()`：场景资源预加载接口，可选实现。
- `canCreateScene()`：用于在创建场景前校验参数，提高鲁棒性。
2. **创建具体适配器**：
适配器用于桥接游戏逻辑层与具体场景类，屏蔽不同场景的创建差异。
```cpp
// 农场场景适配器
...
```
- `registerAdapter()` 由项目初始化阶段统一注册所有场景。
- `createScene()` 会自动找到对应类型的适配器并调用其 `createScene()`。
4. **创建农场场景**:
```cpp
// 使用适配器，统一接口
ValueMap params;
...
* 将事件中心设计成被观察者，负责维护观察者列表，并在事件发布时通知观察者。
* `EventCenter` 被设计为全局单例，确保系统中所有事件源与观察者始终共享同一个事件中心。
...
* 在创建型的重构过程中，生成器模式、工厂模式与对象池模式并不是被孤立地堆叠到系统中，而是围绕角色、动物、作物及工具等核心对象建立起了一套完整的协同流程。动画构建阶段最初的问题在于，诸如 `buildWalkLeft()`、`buildWalkRight()` 等动画创建函数中反复出现相同的帧拼接与延迟设置逻辑，不仅冗余，而且难以统一修改。引入 `AnimationBuilder::from().frames().delay().build()` 之后，所有动画资源都通过标准化链式构建生成，无论是角色走路动画还是动物闲逛动画，都能保持一致的初始化方式，使动画创建本身成为高度规范化的过程。
* 在此基础上，动物与作物的创建环节不再在业务代码中直接调用 `Sheep::create()` 或 `Wheat::create()`，而是交由 `AnimalFactoryManager::createAnimal()` 与 `CropFactoryManager::createCrop()` 统一处理。工厂内部不仅完成了对象的 new/init，还会根据类型为对象绑定由 AnimationBuilder 构建的动画。例如 `SheepFactory::createAnimal()` 内部会自动调用构建好的 `buildDefaultWalkAnimation()` 或设置其 `initState()`。如此一来，动画构建与对象初始化形成自然衔接，业务代码无需关心对象构造细节。
* 然而，在游戏运行时，大量对象的生命周期极短，比如工具使用时的 Sprite、收割作物后的残留节点等。即使工厂创建过程已经被规整，但频繁调用 `Sprite::create()` 仍会导致显著性能抖动。因此，对象池模式成为运行阶段的重要补足。`ToolPool::getPooledTool()` 会优先复用池中的对象，若对象是新的，则仍交由工厂（或原生构造）完成首次初始化；而使用完毕后，则通过 `ToolPool::recycleTool()` 归还并调用 `resetState()`，确保对象状态干净一致。对象池能够运作顺畅的前提，正是工厂构造流程本身是标准化、可预测、可重复的，而这又依赖动画生成器和初始化逻辑的规范化。
* 最终三者之间形成了清晰的分工链：`AnimationBuilder::build()` 负责规范资源构建；`AnimalFactoryManager::createAnimal()` 与 `CropFactoryManager::createCrop()` 负责统一对象初始化；`ToolPool::getPooledTool()` 与 `recycleTool()` 负责运行期的高频复用。构建 → 创建 → 复用 的关系由此自然串联在一起，实现了从资源到对象再到生命周期管理的完整闭环。

### 结构型模式
* 刚开始重构场景系统时，我们优先选择了外观模式（Facade）来对场景切换流程进行统一封装。原因在于，原有代码中场景创建、资源预加载、参数处理以及 `Director` 的切换方法严重散落在业务逻辑中，不仅分布零散，而且高度重复，导致任何场景相关的改动都需要在多个模块中进行同步维护。外观模式能够很好地解决这一问题：通过一个 `GameSceneFacade` 将场景切换流程集中管理，对外暴露一个高度简化的统一入口，使上层逻辑无需接触任何底层实现细节，从而显著降低调用复杂度，提高代码的可读性与一致性。
* 然而，当外观模式初步落地后，我们发现，不同场景内部的创建接口千差万别，有的需要 `mapIndex` 参数，有的需要 `SaveData`，有的甚至要先初始化若干管理器。当外观模式需要直接调用这些具体场景类时，它就不得不包含大量分支判断和特定类型逻辑，使得外观类本身开始变得越来越臃肿，甚至重新陷入对具体场景类的强依赖。更严重的是，新增场景意味着必须修改外观模式内部代码，而这显然违背了开闭原则，也让外观模式难以维持其“稳定入口”的初衷。
* 因此，我们引入了适配器模式（Adapter）与外观模式形成配合。适配器模式将每个场景内部复杂、各自为政的构造逻辑隔离开来，通过统一的 `ISceneCreator` 接口进行封装，从而实现场景创建接口的标准化。外观模式再也不需要了解每个场景的细节，只需根据场景类型调用对应的适配器即可。
* 最终，外观模式负责统一入口、屏蔽复杂性，而适配器模式负责吸收差异、提供统一接口。两者相互协作，让整个场景系统不仅易于调用，也增强了整个系统的扩展性和解耦性。
