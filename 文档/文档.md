...
* 将作物状态变化改为事件发布，将原本直接修改 UI 或场景状态的逻辑移除，改成发布事件。
...
5. **修改 outside 类**：
* 使 `outside` 继承 `Observer`，实现 `onNotify`
```cpp
...
6. **集中 UI 更新逻辑**：
* 将原本分散在各类中的 UI 更新逻辑迁移到 `onNotify`，移除 `outside::updateTime()` 中的 UI 直接更新代码。
...
* 对象池类图
<p align="center">
  <img src="diagram/Action.svg" alt="Presentation Layer" style="width:100%; max-width:1200px; height:auto;"/>
</p>
...
通过引入对象池机制重构后，在角色使用工具、种植作物及收获作物的操作场景中，实现了多维度的优化提升：
...
- **内存层面**：原先依赖autorelease机制管理对象，造成内存管理压力较大；重构后对象分配次数大幅降低（20～60倍），有效缓解了内存波动问题。
...
综上，通过对象池化的改造，在性能、内存、稳定性及可维护性上均实现了优化，尤其在高频操作场景下优势更为突出。


## AI工具使用情况的阐述

### 模式设计的探索和选择
项目初期，我们发现游戏各个模块（如时间、动物、植物）之间存在大量直接调用，代码高度耦合，难以维护。于是，我们向AI描述了这个问题，并询问‘如何实现游戏模块间的事件通知以降低耦合度？”AI向我介绍了观察者模式，并提供了⼀个基本的C++实现框架。在理解了其原理后，我认为它非常适合我的项目，并决定采用它来构建 `EventCenter`。

### 具体代码的重构和生成
旧代码中， `Animal` 类里有大量 `if-else` 语句用于判断和切换动物的状态。我将这段 `if-else` 代码提供给AI，并要求它‘请使用状态模式重构这段代码’。AI⽣成了 AnimalState 接口以及多个具体状态类（`StandingState` , `WalkingState` ）的骨架代码。我在此基础上，结合Cocos2d-x的API，填充了具体的动画和行为逻辑。”

### 反思与讨论
* **最佳实践**: 我们发现，AI最有效的角色是‘结对编程的伙伴’。我们负责提出问题和进⾏⾼层设计，AI负责生成样板代码和提供解决方案选项。”
* **局限性**: “AI无法理解项目的整体架构。它无法主动发现 `outside.cpp` 是⼀个上帝类。它的知识也可能过时，有时会生成不兼容的API调用。”
* **挑战与解决方案**: “最大的挑战是AI生成的代码无法直接在项目中运行。我们必须通过查阅Cocos2d-x官方文档来验证和修正AI给出的API，并通过逐步调试来确保代码的正确性。这让我们认识到，独立思考和验证比单纯地使用AI更重要。”
