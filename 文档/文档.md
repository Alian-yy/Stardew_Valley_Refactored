...
**1. 输入管理模块 (InputManager)**
>>>>>>> 77f1c0c67c33fa273e4dab6c8de7ed112cac408e
...
public:
    ...
    void onMinuteChanged(const std::string& timeStr); // 可由 EventCenter 调用
private:
...
public:
    ...
    void start();
    void stop();
    void update(float dt); // 绑定到 schedule
...
- 负责update 调度（负责 spawn、统一 update、移除）
```cpp
#pragma once
#include "cocos2d.h"
...
创建型的三种模式进行结合：**Builder → Factory → Object Pool**

形成了**从“对象构建”到“对象创建入口”再到“对象生命周期管理”**的一条完整链路：

**1. Builder**

负责把复杂对象（例如动画）构建清晰化、模块化。

**2. Factory**

把所有对象创建统一入口，隐藏具体类型，提升可扩展性。

**3. Object Pool**

将频繁创建的对象复用，加速性能并保证状态安全。

**三者组合形成完整的创建链路，既保证灵活性，又强化扩展性，还强化性能与稳定性。**
