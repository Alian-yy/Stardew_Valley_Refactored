...
* 使用工厂模式重构动物创建系统后，实现了创建逻辑的抽象与解耦，带来以下改进：  

| 问题           | 改进后                      |
| ---------------- | ---------------------- |
| 创建逻辑与具体类耦合 | 通过工厂接口隔离具体类依赖，业务代码仅依赖抽象`AnimalFactory`与`Animal` |
| 新增动物需修改多处代码 | 新增动物只需实现对应具体工厂类并注册到管理器，无需修改原有创建逻辑 |
| 创建流程重复且分散 | 统一由工厂类封装创建细节，工厂管理器集中管理所有工厂 |
| 难以全局控制创建过程 | 可在工厂或管理器中统一添加日志、缓存、权限校验等全局逻辑 |  

有效解决对象创建的耦合问题，提升系统扩展性与可维护性；符合“开闭原则”，新增动物类型时仅需扩展而无需修改原有代码，降低维护成本与出错风险。
